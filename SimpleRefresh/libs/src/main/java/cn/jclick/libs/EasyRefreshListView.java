...
public class EasyRefreshListView extends ListView implements OnScrollListener{
	...
	private LvProgressBar footerPanel;
	 ...
	 * @param attrs
	 */
	public EasyRefreshListView(Context context, AttributeSet attrs) {
		super(context, attrs);
	 ...
	 * 
	 */
	public EasyRefreshListView(Context context) {
		super(context);
		this.mContext = context;
		init(context, null, 0);
	}
	 ...
	 * 
	 */
	public EasyRefreshListView(Context context, AttributeSet attrs, int defStyle) {
	...
	private View initFooterView(Context context){
		...
		footerPanel = (LvProgressBar) footerParent.findViewById(R.id.app_comment_lv_footer);
		...
		if(scrollState == OnScrollListener.SCROLL_STATE_IDLE && view.getLastVisiblePosition() == view.getCount()-1){
			if(this.canLoadMore){
				...
				if(this.mNoMoreHandler == NoMoreHandler.NO_MORE_LOAD_SHOW_TOAST && this.getFirstVisiblePosition() > 0){
					...
					mToast = Toast.makeText(mContext, mContext.getString(R.string.no_more), Toast.LENGTH_SHORT);
					mToast.show();
				}
			}
		}
